...
router.post('/validate-session', async (req, res) => {
    try {
        ...
        // Check device ID - register on first use, enforce on subsequent uses
        ...
        // Auto-register device on first attendance attempt
        if (!req.user.deviceId) {
            const User = require('../models/User');
            await User.findByIdAndUpdate(req.user.id, { deviceId });
        }
